...
concentration.AVG.check<-(Raw.data %>% filter(Sample.ID!="Method Blank", Isotope=="Concentration AVG"))[,c(1,4:11)]

for(i in 1:nrow(concentration.AVG.check))
{
  det<-as.numeric(concentration.AVG.check[i,2:ncol(concentration.AVG.check)]) > 100
  if(!all(det))
  {
    Raw.data[which(Raw.data$Sample.ID==concentration.AVG.check$Sample.ID[i]), which(det==FALSE)+3]<-NA
  }
}
for(i in 1:nrow(concentration.AVG.check))
{
  det<-as.numeric(concentration.AVG.check[i,2:ncol(concentration.AVG.check)]) < 0.1
  if(!all(det))
  {
    Raw.data[which(Raw.data$Sample.ID==concentration.AVG.check$Sample.ID[i]), which(det==FALSE)+3]<-NA
  }
...
all<-rbind(Cd_site_summary,Pb_site_summary,V_site_summary,Cr_site_summary,Mn_site_summary,Ni_site_summary,Cu_site_summary,Zn_site_summary)
all %>% group_by(metal) %>% summarize(soil.diluted=mean(soil.diluted,na.rm=TRUE),soil=mean(soil,na.rm=TRUE),leaf=mean(leaf,na.rm=TRUE),cata=mean(cata,na.rm=TRUE),mean.acc1=mean(acc1,na.rm=TRUE),mean.acc2=mean(acc2,na.rm=TRUE))
