...
tmp<-(Raw.data %>% filter(Sample.ID!="Method Blank", Isotope=="Intensity AVG"))[,c(1,4:11)]

for(i in 1:nrow(tmp))
{
  det<-as.numeric(tmp[i,2:ncol(tmp)]) > blank.intensity * 3
  if(!all(det))
  {
    Raw.data[which(Raw.data$Sample.ID==tmp$Sample.ID[i]), which(det==FALSE)+3]<-NA
  }
}
...
all<-rbind(Cd_site_summary,Pb_site_summary,V_site_summary,Cu_site_summary)
